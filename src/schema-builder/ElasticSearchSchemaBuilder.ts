...
import { ElasticSearchIndexOptions } from "../driver/elasticsearch/typings";
 ...
 */
export class ElasticSearchSchemaBuilder implements SchemaBuilder {
  ...
  async build(): Promise<void> {
    const queryRunner = this.connection.createQueryRunner() as ElasticSearchQueryRunner;
    const promises: Promise<any>[] = [];
    this.connection.entityMetadatas.forEach(metadata => {
      metadata.indices.forEach(index => {
        const options: ElasticSearchIndexOptions = Object.assign({}, {
          name: index.name,
      ...
      metadata.uniques.forEach(unique => {
        const options = <ElasticSearchIndexOptions>{
          name: unique.name,
          unique: true,
        };
